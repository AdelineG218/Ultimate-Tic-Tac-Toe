...
Point LargeBoard::getCoordinatesOfPiece(const int X, const int Y) const {
    Point coord = Point();
    if (X < 300) {
        coord.row = 0;
    } else if (X < 600) {
...
Point LargeBoard::getCoordinatesOfSmallBoard(const int X, const int Y) const {
    Point smallBoard = getCoordinatesOfPiece(X, Y);
    Point coord = Point();
    int x = X-300*smallBoard.col;
    int y = Y-300*smallBoard.row;
